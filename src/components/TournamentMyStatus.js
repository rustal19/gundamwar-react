...
function myResultLabel(match, myEntryId) {
...
export default function TournamentMyStatus({
  ...
  const phase = getMyStatusPhase(tournament, myEntry, rounds);
  ...
  const result = myResultLabel(myMatch, myEntry?.id);
    ...
    return (
      <section className="tournament-my-status-band primary">
        ...
        <div className="tournament-my-status-result">
          {result ? (
            <span className={`tournament-result-badge ${result === "敗北" ? "loss" : "win"}`}>
              結果: {result}
            </span>
          ) : (
