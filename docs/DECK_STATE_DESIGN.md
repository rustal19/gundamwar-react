...
### 4.1.2 チェックイン開始時刻(ユーザー決定 2026-08-26)

現状、セルフチェックインの可否は「開催日当日」+「ラウンド未生成」というハードコードで、
主催者が受付開始時刻を決められない。**ロックの契機をチェックインにした以上、当日0時から
誰でもチェックインできてデッキが固まってしまう**ため、開始時刻を主催者が指定できるようにする。

```sql
ALTER TABLE tournaments
  ADD COLUMN checkin_opens_at DATETIME NULL;   -- NULL = 従来どおり開催日当日0時から
```

- 設定あり: `現在 >= checkin_opens_at` ならセルフチェックイン可(**当日縛りは外す**。
  前日受付なども主催者の判断でできる)
- 設定なし: 従来どおり「開催日当日」のみ
- どちらの場合も「ラウンド未生成」の条件は維持する
- `checkin_opens_at` は開始日時(`starts_at`)より後には設定できない(入力時にエラー)
- 主催者によるチェックイン操作はこの時刻に縛られない(当日の運用判断を優先)
- 大会詳細に「チェックイン開始 ○月○日 10:00」を表示し、開始前は理由が分かるようにする
