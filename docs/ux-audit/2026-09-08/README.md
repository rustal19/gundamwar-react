...
## 9月9日追加調整：保存前のフォーマット・禁止制限

- デッキ側に専用の選択欄を追加。未ログイン・未保存・空デッキでも選択できます。検索側、URL、保存・読み込みと選択値を共有します。
- 既存プリセットの禁止・制限カード154 IDを親リポジトリの `csv/card_data.csv` と照合し、表示専用の名称データを追加。カード未追加・通信なしでも実名で確認できます。禁止制限ルールやAPI契約は変更していません。
- 確認欄は開閉可能。閉じてもフォーマット名・違反件数を残し、枚数変更に追従します。未選択・空デッキ・ルール未登録は適合済みと表示しません。
- 保存済みデッキの公開判定は編集途中の内容と区別し、公開できない理由を維持。違反の詳細も折りたたみ可能です。
- PC 1440px／スマホ390pxで、未ログインの選択、禁止1枚、制限のメイン・サイド合算2枚、違反解消、開閉・Enter操作、未送信検索文字の保持、検索側同期、リセット、保存、再読み込みを実操作で確認。18枚の `format-*.jpg` と `format-flow-checks.json` を保存しました。
- 320／390／768／1100／1101／1440pxで横はみ出しなし。モバイル表示の選択欄は高さ44px・文字16px、開閉ボタンは44px以上です。測定結果は `format-responsive-checks.json`。
- 最終状態で46 suites／526 testsと通常のESLint付き本番用ビルドが成功。比較ページのデッキ構築PC・スマホ画像と `overview.jpg` を更新しました。画像はローカル検証であり、本番への保存・公開・デプロイは行っていません。

## 9月9日追加調整：デッキ合計枚数の折り返し

- `メイン 24/50 ・ サイド 2/10` の表示だけを縮めず一行に保持。横幅が不足する場合は、種類別内訳と合計表示を別行に配置します。文字サイズや枚数の計算は変更していません。
- 320／390／768／1100／1101／1280／1440／1920px × 空・24枚/2枚・上限超過594枚/111枚の24条件で、一行表示・正しい枚数・横はみ出しなしを実ブラウザで確認。526件の自動テストも成功しました。
- `count-nowrap-1440.jpg` / `count-nowrap-390.jpg` に見出し部分の確認画像、`count-nowrap-checks.json` に寸法検証を保存。既存のデッキ構築・フォーマット確認画像と `overview.jpg` も更新しました。画像の枚数例はローカル検証用データです。

## 9月10日追加調整：大会一覧のフィルター位置

- 未ログイン時は「表示範囲」にだけログイン案内が付くため、親の中央揃えによって「ステータス」が9px下がっていました。フィルター2群だけを上揃えにし、案内文、主催者の大会作成ボタン、詳細・運営画面の共通配置は維持しました。
- 320／390／760／761／1100／1101／1440／1920px × 未ログイン・一般ユーザー・主催者の24条件で検証。横並び時のラベル・枠・先頭ボタンの上端差は0px、縦並び時の重なりとページ全体の横はみ出しはありません。ステータス切り替え・自分の大会の絞り込みも確認しました。
- 46 suites／535 testsが成功。`tournament-filter-before-*.jpg` / `tournament-filter-after-*.jpg` に確認画像、`tournament-filter-*-checks.json` に寸法検証を保存。比較ページの大会一覧PC・スマホ画像も更新しました。本番デプロイは行っていません。

## 9月10日追加調整：デッキ提出の構築リンク

- 「デッキ構築へ」はPCで最小高さ36pxに上下余白・枠線が加算され、54pxになっていました。リンクのみ `border-box` に揃え、文字を他の操作ボタンと同じ14pxにして高さ37pxへ調整。横幅・左寄せ・遷移先は維持しています。
- 320／390／760／761／1100／1101／1440／1920pxで、一行ボタンと高さ・文字サイズ・太さが一致。狭い幅で長い選択肢が自然に折り返すことは維持し、横はみ出しなし、現在のデッキ選択、Enterで構築画面へ遷移、未保存デッキ保持を確認しました。
- `deck-choice-before-*.jpg` / `deck-choice-after-*.jpg` と `deck-choice-*-checks.json` に確認画像・実寸を保存。認証・保存デッキはローカル検証用データで、本番への書き込みはありません。

## 9月10日追加調整：縦幅が短いデッキ構築画面

- PCでは全画面高の作業領域の外にフッターがあり、フッターへフォーカスすると、スクロール禁止中でも文書全体が111px上へ移動していました。作業領域とフッターを同じ画面高の中で配分し、フッターを隠さず到達可能にしました。
- 縮小表示の変形前サイズが生む、外側の見えないスクロール領域を解消。デッキ側・検索結果側の内側スクロールは維持し、検索結果領域には最低高を設定しました。PC固定表示へ入る時のみ前画面の残留スクロールを解除し、離脱時は元のスクロール設定へ戻します。
- 幅1100px以下のPC表示もフッターまで画面内へ配置。モバイルOS相当では既存のスクロール方式を維持し、固定ヘッダー52pxを除いた高さを割り当て、下端のはみ出しを防ぎました。
- 1280×750／1280×650／1440×500／1101×400／1920×500／1440×1024／1100×500／844×390／390×500、およびiOS表示指定の390×500／844×390の11条件を検証。画面縮小、フッターフォーカス、一覧最下部の操作、検索、保存画面の開閉、スクロール済み規約ページからの遷移を確認しました。44件の外枠寸法チェックは `deck-height-after-checks.json` に保存しています。
- 全47 suites／558 tests、ESLintを有効にした通常の `CI=true npm run build` が成功。PC・スマホの比較画像と `overview.jpg` を更新しました。`height-final-pages.json` は今回の既定サイズ撮影時の記録です。
- `deck-height-after-*.jpg` は実検索APIから取得したカードを入れたローカル画面です。API応答は読み取り専用で取得し、ローカル検証用に再利用しています。認証とデッキ内容は使い捨てのテストデータであり、デッキの適合性を示すものではありません。`deck-height-before-1280x650.jpg` は空デッキで上部欠けを再現した画像で、前後のカード内容は異なります。
- iOS相当はWindows Edgeでアプリのモバイル表示指定を使った確認で、実機Safariやソフトウェアキーボードの検証ではありません。本番への書き込み・公開・デプロイは行っていません。
- 9月9日のフォーマット検証はその時点の記録です。現行コードではデッキのフォーマットと検索用のフォーマットが分離されており、今回の高さ調整では、その挙動や変更時の確認を変更していません。

## 9月10日追加調整：デッキ検索の画像グリッド

- 画像表示の枚数行が一行固定だったため、内側グリッドが外側の列より広がり、画像・追加ボタン・枚数が隣列へ重なっていました。内側列を収まる幅へ制約し、画像の枠線も幅内へ含め、PCの最小列幅を128pxにしました。情報を切り捨てるのではなく、必要時には枚数を折り返します。
- 画像表示の枚数は「メイン0枚 / サイド0枚」に短縮。0枚の表示と追加時のライブ通知を維持しました。420px以下のデッキ検索だけ2列にし、詳細表示・通常のカード検索・デッキ本体の画像表示は変更していません。
- 1920×900／1440×500／1280×650／1101×500／1100×650／768×650／421×650／420×650／360×600／390×600／320×600と、iOS表示指定の390×600の12条件で50件を表示。追加前・メイン/サイド追加後の24状態で列内のはみ出し・ボタン文字の折り返し・横スクロールは0件。詳細/画像切替、枚数更新、末尾カードのボタン到達も確認しました。
- 全47 suites／559 testsと、ESLint有効の通常の本番用ビルドが成功。寸法は `deck-image-before-checks.json` / `deck-image-after-checks.json`、画像は `deck-image-before-*.jpg` / `deck-image-after-*.jpg` に保存しています。修正前は9条件、修正後は境界幅を加えた12条件です。
- カードは実検索APIの読み取り専用応答をローカル検証へ再利用しています。デッキ追加は使い捨てブラウザ内のみ。スマホの修正後画像はカードと操作が見えるよう検索領域をスクロールした位置で撮影しました。iOS相当はEdge上のアプリ表示指定であり、実機ではありません。本番への書き込み・公開・デプロイは行っていません。
